...
class floorplanner:
  ...
  def prepare_fimp_variables(self):
    ...
    self.x = [
      self.solver.IntVar(0, self.cgra.width - self.w[f], 'x[%i]' % f) for f in fimps]
    self.y = [self.solver.IntVar(
      0, self.cgra.height - self.h[f], 'y[%i]' % f) for f in fimps]
